...
const oddFunction = nowNumber => nowNumber % 2 !== 0;

const oddRandomNumbers = randomNumbers.filter( oddFunction );
...
const countFunction = (ac, cnumber) => cnumber < 501 ? ++ac : ac;

const qntCrazyNumbers = crazyNumbers.reduce( countFunction, 0 );
console.log(qntCrazyNumbers);
...
const later2000 = tarantinoMovies.filter( ({ release }) => release < 2000);
console.log(later2000);
...
const cart = [
  ...
  //  - Nome 2
  //  - Nome 3
const listCartFunction = (ac, { name } ) => ac += `- ${name} \n`;
const listCart = cart.reduce( listCartFunction, '');
